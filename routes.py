...
def download_personalized_template(quiz_id, student_id):
    ...
    try:
        ...
        c.drawCentredString(width/2, height - 1*inch, quiz.title)
        ...
        c.drawString(2.5*inch, name_y, student.student_id)

        id_y = height - 2.2*inch
        c.drawString(1.3*inch, id_y, "Name:")
        c.drawString(2*inch, id_y, student.name)

        # Generate QR code with student info
        import qrcode
        from io import BytesIO

        # Create QR code with quiz info
        qr = qrcode.QRCode(version=1, box_size=10, border=5)
        qr_data = f"name:{student.name};id:{student.student_id};quiz:{quiz.title};answers:{quiz.answer_key}"
        qr.add_data(qr_data)
        qr.make(fit=True)

        # Create temporary file for QR code
        temp_qr_path = os.path.join(app.config['UPLOAD_FOLDER'], f'temp_qr_{uuid.uuid4().hex}.png')
        ...
        # Add QR code to PDF
        c.drawImage(temp_qr_path, width-2.3*inch, height-2.35*inch, width=0.75*inch, height=0.75*inch)
        ...
        c.drawString(width - 3.3*inch, section_y, student.section.name)
        ...
        # Draw answer bubbles with improved layout
        start_y = height - 3.8*inch
        row_height = 0.3*inch  # Increased spacing between rows
        bubble_radius = 6  # Slightly larger bubbles
        ...
        for col in range(num_columns):
            ...
            for i in range(start_num, end_num + 1):
                ...
                # Question number with better alignment
                c.setFont("Helvetica-Bold", 10)
                c.drawRightString(col_x + 0.3*inch, y + 0.07*inch, f"{i}.")

                # Answer bubbles with improved spacing
                bubble_x = col_x + 0.5*inch
                bubble_radius = 7 if quiz.num_items == 100 else 5  # Larger bubbles for 100 questions
                bubble_y_offset = 0.08 * inch
                for j, letter in enumerate(['A', 'B', 'C', 'D']):
                    # Draw bubble with adjusted spacing
                    bubble_center_x = bubble_x + j * bubble_spacing
                    c.circle(bubble_center_x, y + bubble_y_offset, bubble_radius, stroke=1, fill=0)
                    # Draw letter inside bubble with better vertical centering
                    c.setFont("Helvetica", 8)
                    letter_height = 8  # Font size in points
                ...
                # Removed grey line for cleaner look
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            for row in csv_reader:
                if len(row) >=2:
                    # Check if first column looks like a student ID
                    if any(x in row[0] for x in ['-S', '-s']):
                        # First column is ID, second is name
                        student_id = row[0].strip()
                        name = row[1].strip()
                    else:
                        # First column is name, second is ID
                        name = row[0].strip()
            ...
            db.session.commit()
            flash('Studentsimported successfully', 'success')
        except Exception as e:
